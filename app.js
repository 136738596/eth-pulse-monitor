...
function refreshLemonButtons() {
  ...
  if (configuredCount === Object.keys(CONFIG.checkoutLinks).length) {
    ...
    checkoutMessage.textContent =
      "Lemon Squeezy links are configured. Customers can pay directly from this page.";
  } else {
    platformStatus.textContent =
      "Lemon Squeezy is selected. Add your three hosted checkout URLs in app.js to go live.";
    checkoutMessage.innerHTML =
      "Recommended setup: create 3 products in Lemon Squeezy, copy each hosted checkout URL, and paste them into <code>CONFIG.checkoutLinks</code>. Until then, the page works as a public sales entry and contact funnel.";
  }
}
...
function attachButtonHandlers() {
  buyButtons.forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      checkoutMessage.innerHTML = `${plan.label} ${plan.price} is ready as an offer, but the checkout URL is still empty. Add the Lemon Squeezy hosted checkout link for <code>${planKey}</code> in <code>app.js</code> to start charging publicly.`;
